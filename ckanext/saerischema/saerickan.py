...
# Configuration:
# Default location of the CSV file which maps topic_category to group name and description
...
DEFAULT_GROUP_CSV_FILENAME = "/usr/lib/ckan/default/src/ckanext-saeritheme/ckanext/saeritheme/tools/topic_categories.csv"
...
def saerickan_convert_bbox_to_geojson(srs, top, bottom, left, right):
    ...
    if srs == 'WGS84' or srs == 'WGS84 ':
        ...
        if top < -90 or top > 90 or bottom < -90 or bottom > 90:
            log.error("saerickan_convert_bbox_to_geojson ERROR out of range1 in %s %s %s %s %s" % (
            srs, top, bottom, left, right))
            return ''
        if left < -180 or left > 180 or right < -180 or right > 180:
            log.error("saerickan_convert_bbox_to_geojson ERROR out of range2 in %s %s %s %s %s" % (
            srs, top, bottom, left, right))
            return ''
    ...
    if src_proj:
        try:
            ...
            if tl_lat < -90 or tl_lat > 90:
                log.error("saerickan_convert_bbox_to_geojson ERROR %s >90 in %s %s %s %s %s" % (
                tl_lat, srs, top, bottom, left, right))
                return ''
            if br_lat < -90 or br_lat > 90:
                log.error("saerickan_convert_bbox_to_geojson ERROR %s >90 in %s %s %s %s %s" % (
                br_lat, srs, top, bottom, left, right))
                return ''
            if tl_lon < -180 or tl_lon > 180:
                log.error("saerickan_convert_bbox_to_geojson ERROR %s >180 in %s %s %s %s %s" % (
                tl_lon, srs, top, bottom, left, right))
                return ''
            if br_lon < -180 or br_lon > 180:
                log.error("saerickan_convert_bbox_to_geojson ERROR %s >180 in %s %s %s %s %s" % (
                br_lon, srs, top, bottom, left, right))
                return ''
            if tl_lon == 0 or tl_lat == 0 or br_lon == 0 or br_lat == 0:
                log.error("saerickan_convert_bbox_to_geojson ERROR zero(s) in %s %s %s %s %s" % (
                srs, top, bottom, left, right))
                return ''
            # Convert to GeoJSON
...
def saerickan_create_mapping_topic_category_to_group():
    ...
    group_csv_filename = config.get('ckan.saerischema.group_csv', DEFAULT_GROUP_CSV_FILENAME)
